function y = nonLinString (f0, d, g, a1, a2)
	...
	for i=1:length(y)
        ...
        delPast = del(L); %store past input sample x[n-1]
		nonL(2) = nonL(1); % shifting the values in the buffer
		nonL(1) = a0*del(L)+delPast-a0*nonL(2); %reading the delay lines last sample(x[n]) + its past sample(x[n-1])

        %coefficient modulation
		w(1) = del(L)-a0*nonL(1);
		%f = (1/(1-a0))*(w(1)+w(2));
	    if sign(w(1)) == 1 %change the filter coefficient on sign change
	        a0 = k(1);
	    elseif sign(w(1)) == -1
	        a0 = k(2);
	    end
